...
void Calam::putFillCircle(vec2D center, int radius, RGBA c) {
  int x = 0;
  int y = radius;
  int p0 = 1 - radius;

  while (x <= y) {
    putLine({center.x - x, center.y + y}, {center.x + x, center.y + y}, c);
    putLine({center.x - x, center.y - y}, {center.x + x, center.y - y}, c);
    putLine({center.x - y, center.y + x}, {center.x + y, center.y + x}, c);
    putLine({center.x - y, center.y - x}, {center.x + y, center.y - x}, c);
